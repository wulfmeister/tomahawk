    ...
    foreach ( const dyncontrol_ptr& c, controls )
    {
        if ( c->selectedType() == "Artist" )
        {
            if ( c->match().toInt() == Echonest::DynamicPlaylist::ArtistType )
                urlQuery.addQueryItem( "artist_limitto", c->input() );
            else
                urlQuery.addQueryItem( "artist", c->input() );
        }
        else if ( c->selectedType() == "Artist Description" )
        {
            urlQuery.addQueryItem( "description", c->input() );
        }
        else
        {
            ...
            urlQuery.addQueryItem( name, c->input() );
        }
    }
...
GlobalActionManager::parseTomahawkLink( const QString& urlIn )
{
    ...
    if ( url.contains( "tomahawk://" ) )
    {
        ...
        if ( cmdType == "load" )
        {
            if ( QUrlQuery( u ).hasQueryItem( "xspf" ) )
            {
                QUrl xspf = QUrl::fromUserInput( QUrlQuery( u ).queryItemValue( "xspf" ) );
            ...
            else if ( QUrlQuery( u ).hasQueryItem( "jspf" ) )
            {
                QUrl jspf = QUrl::fromUserInput( QUrlQuery( u ).queryItemValue( "jspf" ) );
...
GlobalActionManager::handlePlaylistCommand( const QUrl& url )
{
    ...
    if ( parts[ 0 ] == "import" )
    {
        if ( !QUrlQuery( url ).hasQueryItem( "xspf" ) && !QUrlQuery( url ).hasQueryItem( "jspf") )
        {
            tDebug() << "No xspf or jspf to load...";
            return false;
        }
        if ( QUrlQuery( url ).hasQueryItem( "xspf" ) )
        {
            createPlaylistFromUrl( "xspf", QUrlQuery( url ).queryItemValue( "xspf" ), QUrlQuery( url ).hasQueryItem( "title" ) ? QUrlQuery( url ).queryItemValue( "title" ) : QString() );
            return true;
        }
        else if ( QUrlQuery( url ).hasQueryItem( "jspf" ) )
        {
            createPlaylistFromUrl( "jspf", QUrlQuery( url ).queryItemValue( "jspf" ), QUrlQuery( url ).hasQueryItem( "title" ) ? QUrlQuery( url ).queryItemValue( "title" ) : QString() );
            return true;
        }
    }
    else if ( parts [ 0 ] == "new" )
    {
        if ( !QUrlQuery( url ).hasQueryItem( "title" ) )
        {
            tLog() << "New playlist command needs a title...";
            return false;
        }
        playlist_ptr pl = Playlist::create( SourceList::instance()->getLocal(), uuid(), QUrlQuery( url ).queryItemValue( "title" ), QString(), QString(), false );
        ViewManager::instance()->show( pl );
    }
    else if ( parts[ 0 ] == "add" )
    {
        if ( !QUrlQuery( url ).hasQueryItem( "playlistid" ) || !QUrlQuery( url ).hasQueryItem( "title" ) || !QUrlQuery( url ).hasQueryItem( "artist" ) )
        {
...
GlobalActionManager::handleSearchCommand( const QUrl& url )
{
    ...
    QString queryStr;
    if ( QUrlQuery( url ).hasQueryItem( "query" ) )
        queryStr = QUrlQuery( url ).queryItemValue( "query" );
    else
    {
        QStringList query;
        if ( QUrlQuery( url ).hasQueryItem( "artist" ) )
            query << QUrlQuery( url ).queryItemValue( "artist" );
        if ( QUrlQuery( url ).hasQueryItem( "album" ) )
            query << QUrlQuery( url ).queryItemValue( "album" );
        if ( QUrlQuery( url ).hasQueryItem( "title" ) )
            query << QUrlQuery( url ).queryItemValue( "title" );
        queryStr = query.join( " " );
    }
...
//         const QString artist = QUrl::fromPercentEncoding( url.encodedQueryItemValue( "name" ) ).replace( "+", " " );
        const QString artist = QUrlQuery( url ).queryItemValue( "name" );
        if ( artist.isEmpty() )
        {
...
//         const QString album = QUrl::fromPercentEncoding( url.encodedQueryItemValue( "name" ) ).replace( "+", " " );
        const QString artist = QUrlQuery( url ).queryItemValue( "artist" );
        const QString album = QUrlQuery( url ).queryItemValue( "name" );
    ...
    {
//         const QString artist = QUrl::fromPercentEncoding( url.queryItemValue( "artist" ) ).replace( "+", " " );
//         const QString album = QUrl::fromPercentEncoding( url.queryItemValue( "album" ) ).replace( "+", " " );
//         const QString track = QUrl::fromPercentEncoding( url.queryItemValue( "name" ) ).replace( "+", " " );
        const QString artist = QUrlQuery( url ).queryItemValue( "artist" );
        const QString album = QUrlQuery( url ).queryItemValue( "album" );
        const QString track = QUrlQuery( url ).queryItemValue( "track" );
...
GlobalActionManager::loadDynamicPlaylist( const QUrl& url, bool station )
{
    ...
    if ( parts[ 0 ] == "create" )
    {
        if ( !QUrlQuery( url ).hasQueryItem( "title" ) || !QUrlQuery( url ).hasQueryItem( "type" ) )
        {
            tLog() << "Station create command needs title and type..." << url.toString();
            return Tomahawk::dynplaylist_ptr();
        }
        QString title = QUrlQuery( url ).queryItemValue( "title" );
        QString type = QUrlQuery( url ).queryItemValue( "type" );
        ...
        foreach ( param, QUrlQuery( url ).queryItems() )
        {
...
{
    if ( !QUrlQuery( url ).hasQueryItem( "spotifyURI" ) && !QUrlQuery( url ).hasQueryItem( "spotifyURL" ) )
        return false;

    QString spotifyUrl = QUrlQuery( url ).hasQueryItem( "spotifyURI" ) ? QUrlQuery( url ).queryItemValue( "spotifyURI" ) : QUrlQuery( url ).queryItemValue( "spotifyURL" );
...
{
    if ( !QUrlQuery( url ).hasQueryItem( "rdioURI" ) && !QUrlQuery( url ).hasQueryItem( "rdioURL" ) )
        return false;

    QString rdioUrl = QUrlQuery( url ).hasQueryItem( "rdioURI" ) ? QUrlQuery( url ).queryItemValue( "spotifyURI" ) : QUrlQuery( url ).queryItemValue( "rdioURL" );
